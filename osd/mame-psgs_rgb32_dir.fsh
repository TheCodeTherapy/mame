...
vec3 find_closest (vec3 ref, float mode) {
  ...
  if (mode == 1.0) {
    ...
    //try(vec3(146.0, 146.0, 146.0)); // 10 - gray #2      (YPbPr = 0.5  ,  0.0 ,  0.0)
  ...
  if (mode == 4.0) {
    //try(vec3(  0.0,   0.0,   0.0)); //  0 - transparent  (YPbPr = 0.0  ,  0.0   ,  0.0  )
    try(vec3(  0.0,   0.0,   0.0));   //  1 - black        (YPbPr = 0.0  ,  0.0   ,  0.0  )
  ...
  if (mode == 5.0) {
    try(vec3(  0.0,   0.0,   0.0));   //  0 - black
    try(vec3(  0.0, 170.0, 170.0));   //  1 - low intensity cyan
    try(vec3(170.0,   0.0, 170.0));   //  2 - low intensity magenta
    try(vec3(170.0, 170.0, 170.0));   //  3 - low intensity white / light grey
    try(vec3( 85.0, 255.0, 255.0));   //  4 - high intensity cyan
    try(vec3(255.0,  85.0, 255.0));   //  5 - high intensity magenta
    try(vec3(255.0, 255.0, 255.0));   //  6 - high intensity grey / bright white
  }

  // 16-color RGBi IBM CGA as seen on registers from compatible monitors back then
  if (mode == 6.0) {
    try(vec3(  0.0,   0.0,   0.0));   //  0 - black
    try(vec3(  0.0,  25.0, 182.0));   //  1 - low blue
    try(vec3(  0.0, 180.0,  29.0));   //  2 - low green
    try(vec3(  0.0, 182.0, 184.0));   //  3 - low cyan
    try(vec3(196.0,  31.0,  12.0));   //  4 - low red
    try(vec3(193.0,  43.0, 182.0));   //  5 - low magenta
    try(vec3(193.0, 106.0,  21.0));   //  6 - brown
    try(vec3(184.0, 184.0, 184.0));   //  7 - light grey
    try(vec3(104.0, 104.0, 104.0));   //  8 - dark grey
    try(vec3( 95.0, 110.0, 252.0));   //  9 - high blue
    try(vec3( 57.0, 250.0, 111.0));   // 10 - high green
    try(vec3( 36.0, 252.0, 254.0));   // 11 - high cyan
    try(vec3(255.0, 112.0, 106.0));   // 12 - high red
    try(vec3(255.0, 118.0, 253.0));   // 13 - high magenta
    try(vec3(255.0, 253.0, 113.0));   // 14 - yellow
    try(vec3(255.0, 255.0, 255.0));   // 15 - white
  }

  // Part three - my hand crafted palettes ==========================================================================

  if (mode == 7.0) {                  // 54 COLORS NESish palette
    try(vec3(  0.0,   0.0,   0.0));   //vec3 (  0.0,   88.0,   0.0)
    try(vec3( 80.0,  48.0,   0.0));
    try(vec3(  0.0, 104.0,   0.0));
    try(vec3(  0.0,  64.0,  88.0));
    try(vec3(  0.0, 120.0,   0.0));
    try(vec3(136.0,  20.0,   0.0));
    try(vec3(  0.0, 168.0,   0.0));
    try(vec3(168.0,  16.0,   0.0));
    try(vec3(168.0,   0.0,  32.0));
    try(vec3(  0.0, 168.0,  68.0));
    try(vec3(  0.0, 184.0,   0.0));
    try(vec3(  0.0,   0.0, 188.0));
    try(vec3(  0.0, 136.0, 136.0));
    try(vec3(148.0,   0.0, 132.0));
    try(vec3( 68.0,  40.0, 188.0));
    try(vec3(120.0, 120.0, 120.0));
    try(vec3(172.0, 124.0,   0.0));
    try(vec3(124.0, 124.0, 124.0));
    try(vec3(228.0,   0.0,  88.0));
    try(vec3(228.0,  92.0,  16.0));
    try(vec3( 88.0, 216.0,  84.0));
    try(vec3(  0.0,   0.0, 252.0));
    try(vec3(248.0,  56.0,   0.0));
    try(vec3(  0.0,  88.0, 248.0));
    try(vec3(  0.0, 120.0, 248.0));
    try(vec3(104.0,  68.0, 252.0));
    try(vec3(248.0, 120.0,  88.0));
    try(vec3(216.0,   0.0, 204.0));
    try(vec3( 88.0, 248.0, 152.0));
    try(vec3(248.0,  88.0, 152.0));
    try(vec3(104.0, 136.0, 252.0));
    try(vec3(252.0, 160.0,  68.0));
    try(vec3(248.0, 184.0,   0.0));
    try(vec3(184.0, 248.0,  24.0));
    try(vec3(152.0, 120.0, 248.0));
    try(vec3(  0.0, 232.0, 216.0));
    try(vec3( 60.0, 188.0, 252.0));
    try(vec3(188.0, 188.0, 188.0));
    try(vec3(216.0, 248.0, 120.0));
    try(vec3(248.0, 216.0, 120.0));
    try(vec3(248.0, 164.0, 192.0));
    try(vec3(  0.0, 252.0, 252.0));
    try(vec3(184.0, 184.0, 248.0));
    try(vec3(184.0, 248.0, 184.0));
    try(vec3(240.0, 208.0, 176.0));
    try(vec3(248.0, 120.0, 248.0));
    try(vec3(252.0, 224.0, 168.0));
    try(vec3(184.0, 248.0, 216.0));
    try(vec3(216.0, 184.0, 248.0));
    try(vec3(164.0, 228.0, 252.0));
    try(vec3(248.0, 184.0, 248.0));
    try(vec3(248.0, 216.0, 248.0));
    try(vec3(248.0, 248.0, 248.0));
    try(vec3(252.0, 252.0, 252.0));
  }

  else if (mode == 8.0) {             // 38 COLORS
    try(vec3(255.0, 153.0, 153.0));   // L80
    try(vec3(255.0, 181.0, 153.0));   // L80
    try(vec3(254.0, 255.0, 153.0));   // L80
    try(vec3(181.0, 255.0, 153.0));   // L80
    try(vec3(153.0, 214.0, 255.0));   // L80
    try(vec3(153.0, 163.0, 255.0));   // L80
    try(vec3(255.0,  50.0,  50.0));   // L60
    try(vec3(255.0, 108.0,  50.0));   // L60
    try(vec3(254.0, 255.0,  50.0));   // L60
    try(vec3(108.0, 255.0,  50.0));   // L60
    try(vec3( 50.0, 173.0, 255.0));   // L60
    try(vec3( 50.0,  71.0, 255.0));   // L60
    try(vec3(204.0,   0.0,   0.0));   // L40
    try(vec3(204.0,  57.0,   0.0));   // L40
    try(vec3(203.0, 204.0,   0.0));   // L40
    try(vec3( 57.0, 204.0,   0.0));   // L40
    try(vec3(  0.0, 122.0, 204.0));   // L40
    try(vec3(  0.0,  20.0, 204.0));   // L40
    try(vec3(102.0,   0.0,   0.0));   // L20
    try(vec3(102.0,  28.0,   0.0));   // L20
    try(vec3(101.0, 102.0,   0.0));   // L20
    try(vec3( 28.0, 102.0,   0.0));   // L20
    try(vec3(  0.0,  61.0, 102.0));   // L20
    try(vec3(  0.0,  10.0, 102.0));   // L20
    try(vec3(255.0, 255.0, 255.0));   // L100
    ...
    try(vec3(  0.0,   0.0,   0.0));   // L0
    try(vec3(  0.0,   0.0, 255.0));   // L30
    try(vec3(255.0,   0.0,   0.0));   // L54
    try(vec3(  0.0, 255.0,   0.0));   // L88
  }

  else if (mode == 9.0) {             // 16 COLORS
    try(vec3(  0.0,   0.0,   0.0));
    try(vec3(255.0, 255.0, 255.0));
    try(vec3(255.0,   0.0,   0.0));
    try(vec3(  0.0, 255.0,   0.0));
    try(vec3(  0.0,   0.0, 255.0));
    try(vec3(255.0, 255.0,   0.0));
    try(vec3(  0.0, 255.0, 255.0));
    try(vec3(255.0,   0.0, 255.0));
    try(vec3(128.0,   0.0,   0.0));
    try(vec3(  0.0, 128.0,   0.0));
    try(vec3(  0.0,   0.0, 128.0));
    try(vec3(128.0, 128.0,   0.0));
    try(vec3(  0.0, 128.0, 128.0));
    try(vec3(128.0,   0.0, 128.0));
    try(vec3(128.0, 128.0, 128.0));
    try(vec3(255.0, 128.0, 128.0));
  }

  else if (mode == 10.0) {            // 16 COLORS
    try(vec3(  0.0,   0.0,   0.0));
    try(vec3(255.0, 255.0, 255.0));
    try(vec3(116.0,  67.0,  53.0));
    try(vec3(124.0, 172.0, 186.0));
    try(vec3(123.0,  72.0, 144.0));
    try(vec3(100.0, 151.0,  79.0));
    try(vec3( 64.0,  50.0, 133.0));
    try(vec3(191.0, 205.0, 122.0));
    try(vec3(123.0,  91.0,  47.0));
    try(vec3( 79.0,  69.0,   0.0));
    try(vec3(163.0, 114.0, 101.0));
    try(vec3( 80.0,  80.0,  80.0));
    try(vec3(120.0, 120.0, 120.0));
    try(vec3(164.0, 215.0, 142.0));
    try(vec3(120.0, 106.0, 189.0));
    try(vec3(159.0, 159.0, 150.0));
  }

  else if (mode == 11.0) {            // 16 COLORS					
    try(vec3(  0.0,   0.0,   0.0));
    try(vec3(255.0, 255.0, 255.0));
    try(vec3(152.0,  75.0,  67.0));
    try(vec3(121.0, 193.0, 200.0));
    try(vec3(155.0,  81.0, 165.0));
    try(vec3(202.0, 160.0, 218.0));
    try(vec3(202.0, 160.0, 218.0));
    try(vec3(202.0, 160.0, 218.0));
    try(vec3(202.0, 160.0, 218.0));
    try(vec3(191.0, 148.0, 208.0));
    try(vec3(179.0, 119.0, 201.0));
    try(vec3(167.0, 106.0, 198.0));
    try(vec3(138.0, 138.0, 138.0));
    try(vec3(163.0, 229.0, 153.0));
    try(vec3(138.0, 123.0, 206.0));
    try(vec3(173.0, 173.0, 173.0));
  }
    ...
    try(vec3(255.0, 255.0,   0.0));
    try(vec3(  0.0, 255.0, 255.0));
    try(vec3(215.0,   0.0,   0.0));
    try(vec3(  0.0, 215.0,   0.0));
    try(vec3(  0.0,   0.0, 215.0));
    try(vec3(215.0,   0.0, 215.0));
    try(vec3(215.0, 215.0,   0.0));
    try(vec3(  0.0, 215.0, 215.0));
    try(vec3(215.0, 215.0, 215.0));
    try(vec3( 40.0,  40.0,  40.0));
  }

  else if (mode == 13.0) {            // 13 COLORS
    try(vec3(  0.0,   0.0,   0.0));
    try(vec3(  1.0,   3.0,  31.0));
    try(vec3(  1.0,   3.0,  53.0));
    try(vec3( 28.0,   2.0,  78.0));
    try(vec3( 80.0,   2.0, 110.0));
    try(vec3(143.0,   3.0, 133.0));
    try(vec3(181.0,   3.0, 103.0));
    try(vec3(229.0,   3.0,  46.0));
    try(vec3(252.0,  73.0,  31.0));
    try(vec3(253.0, 173.0,  81.0));
    try(vec3(254.0, 244.0, 139.0));
    try(vec3(239.0, 254.0, 203.0));
    try(vec3(242.0, 255.0, 236.0));
  }

  else if (mode == 14.0) {            // 5 COLORS (GREENISH - GAMEBOY)
    try(vec3( 41.0,  57.0,  65.0));
    try(vec3( 72.0,  93.0,  72.0));
    try(vec3(133.0, 149.0,  80.0));
    try(vec3(186.0, 195.0, 117.0));
    try(vec3(242.0, 239.0, 231.0));
  }

  else if (mode == 15.0) {            // 5 COLORS (PURPLEISH)
    try(vec3( 65.0,  49.0,  41.0));
    try(vec3( 93.0,  72.0,  93.0));
    try(vec3( 96.0,  80.0, 149.0));
    try(vec3(126.0, 117.0, 195.0));
    try(vec3(231.0, 234.0, 242.0));
  }

  else if (mode == 16.0) {            // 4 COLORS (GREENISH)
    try(vec3 (156.0, 189.0,  15.0));
    try(vec3 (140.0, 173.0,  15.0));
    try(vec3 ( 48.0,  98.0,  48.0));
    try(vec3 ( 15.0,  56.0,  15.0));
  }
  ...
  mix(
    mix(
      mix(
        mix(
          mix(
            mix(0.0,32.0,step(1.0,y)),
            mix(8.0,40.0,step(3.0,y)),
            step(2.0,y)
          ),
          mix(
            mix(2.0,34.0,step(5.0,y)),
        ...
        mix(
          mix(
            mix(48.0,16.0,step(1.0,y)),
            mix(56.0,24.0,step(3.0,y)),
            step(2.0,y)
          ),
          mix(
            mix(50.0,18.0,step(5.0,y)),
      ...
      mix(
        mix(
          mix(
            mix(12.0,44.0,step(1.0,y)),
            mix(4.0,36.0,step(3.0,y)),
            step(2.0,y)
          ),
          mix(
            mix(14.0,46.0,step(5.0,y)),
        ...
        mix(
          mix(
            mix(60.0,28.0,step(1.0,y)),
            mix(52.0,20.0,step(3.0,y)),
            step(2.0,y)
          ),
          mix(
            mix(62.0,30.0,step(5.0,y)),
    ...
    mix(
      mix(
        mix(
          mix(
            mix(3.0,35.0,step(1.0,y)),
            mix(11.0,43.0,step(3.0,y)),
            step(2.0,y)
          ),
          mix(
            mix(1.0,33.0,step(5.0,y)),
        ...
        mix(
          mix(
            mix(51.0,19.0,step(1.0,y)),
            mix(59.0,27.0,step(3.0,y)),
            step(2.0,y)
          ),
          mix(
            mix(49.0,17.0,step(5.0,y)),
      ...
      mix(
        mix(
          mix(
            mix(15.0,47.0,step(1.0,y)),
            mix(7.0,39.0,step(3.0,y)),
            step(2.0,y)
          ),
          mix(
            mix(13.0,45.0,step(5.0,y)),
        ...
        mix(
          mix(
            mix(63.0,31.0,step(1.0,y)),
            mix(55.0,23.0,step(3.0,y)),
            step(2.0,y)
          ),
          mix(
            mix(61.0,29.0,step(5.0,y)),
...
void main() {  
  ...
  vec3 blur = mix(
    mix(
      TEX2DH(xy),
      TEX2DH(xy+vec2(one.x,0.0)),
      uv_ratio.x
    ),
    mix(
      TEX2DH(xy+vec2(0.0,one.y)),
      TEX2DH(xy+one),
      uv_ratio.x
    ),
    uv_ratio.y
